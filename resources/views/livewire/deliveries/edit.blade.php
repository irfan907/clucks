<section class="w-full">
    <div class="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <flux:heading class="text-lg sm:text-xl">{{ __('Edit Delivery') }}</flux:heading>
    ...
    @if($currentCategory)
        <form wire:submit="save" class="space-y-4">
            ...
            <div class="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
                <flux:checkbox wire:model.live="showAllProducts" :label="__('Show All Products (including those not in this delivery)')" />
            </div>
                                            ...
                                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                                                ...
                                                <td class="px-2 py-2">
                                                    <input 
                                                        ...
                                                        step="0.01" 
                                                        min="0"
                                                        wire:model="items.{{ $row['category_id'] }}.{{ $row['product_id'] }}.current_stock"
                                                ...
                                                <td class="px-2 py-2">
                                                    <input 
                                                        ...
                                                        step="0.01" 
                                                        min="0"
                                                        wire:model="items.{{ $row['category_id'] }}.{{ $row['product_id'] }}.quantity"
                                                        value="{{ $row['quantity'] }}"
                                                        placeholder="0"
                                                        class="w-full px-1.5 py-1 text-xs border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 focus:ring-1 focus:ring-accent focus:border-transparent"
                                                    />
                                                ...
                                                <td class="px-2 py-2">
                                                    <input 
                                                        type="text" 
                                                        wire:model="items.{{ $row['category_id'] }}.{{ $row['product_id'] }}.note"
            ...
            <div class="flex flex-col gap-3 border-t border-zinc-200 pt-4 dark:border-zinc-700 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                    <flux:button type="button" :href="route('deliveries.show', $delivery)" wire:navigate variant="ghost" class="w-full sm:w-auto">{{ __('Cancel') }}</flux:button>
                    @if(!$isLastCategory)
                        <flux:button type="button" wire:click="goToNextCategory" variant="outline" class="w-full sm:w-auto">{{ __('Next') }}</flux:button>
                    @endif
                    @if(!$isFirstCategory)
                        <flux:button type="button" wire:click="goToPreviousCategory" variant="outline" class="w-full sm:w-auto">{{ __('Previous') }} </flux:button>
                    @endif
                </div>
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                    <flux:button type="button" wire:click="save" variant="primary" class="w-full sm:w-auto">{{ __('Save') }}</flux:button>
                    <flux:button type="button" wire:click="saveAndGoBack" variant="primary" class="w-full sm:w-auto">{{ __('Save & Go Back') }}</flux:button>
                </div>
            </div>
        </form>
    @else
